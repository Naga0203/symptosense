...
class OpenRouterService:
    def __init__(self):
        ...
        # Primary model from .env with reliable fallbacks
    ...
    def extract_validated_symptoms(self, user_input: str) -> dict:
        """
        Extract and validate symptoms using a strict KNOWN_SYMPTOMS_LIST.
        """
        prompt = f"""SYSTEM:
You are a clinical symptom validation and prediction engine.
You must strictly follow the rules and return ONLY valid JSON.
...
2. Normalize them into standard medical terms.
3. Check each normalized symptom against KNOWN_SYMPTOMS_LIST.
4. If a symptom is NOT in the list, mark it as "invalid".
5. If ALL symptoms are invalid, return "invalid_symptom" as the final result.
6. If there are valid symptoms:
   - Use ONLY the valid symptoms
   - Predict related symptoms (from KNOWN_SYMPTOMS_LIST only)
   - Do NOT introduce new symptoms outside the list
  ...
  "status": "valid" | "invalid_symptom"
        ...
        for model_name in self.models:
            try:
                ...
                print(f"Error calling model {model_name} for symptom extraction: {e}")
                continue
        
        return {
            "status": "invalid_symptom",
            "extracted_symptoms": [],
            "valid_symptoms": [],
            "invalid_symptoms": ["Failed to process symptom data."]
        }
